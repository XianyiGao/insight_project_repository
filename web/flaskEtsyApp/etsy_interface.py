"""
This module contains one class Etsy, acting as an interface
to connect with Etsy API.
"""
...
class Etsy(object):
    ...
    def get_auth_token(self, verifier, oauth_token, oauth_token_secret):
        ...
        parsed = urllib.parse.parse_qs(response.text)
        return {'oauth_token': parsed['oauth_token'][0],
    ...
    def execute(self, endpoint, method='get', oauth=None):
        ...
        if response.status_code > 201:
            ...
            raise self.EtsyError('API returned %s response: %s' % (code, error_text))
        return response
